  ...
  <div>
    ...
    <v-dialog :isShow="modelValue" @close="closeFormHandle">
      <template #title>
        <div class="row e-header">
          <div class="e-header__title col font-weight-700">
            {{
              isAdd
                ? $t("table_detail.title_add")
                : $t("table_detail.title_edit")
            }}
          </div>
        </div>
      </template>
          ...
          <div class="col l-6 md-6">
            <div class="row sm-gutter">
              <div class="form-group col l-5 md-5 c-5 focus">
                <v-input
                  :label="$t('table_detail.code')"
                  v-model="tableManger.tableManagerCode"
                  ref="tableManagerCode"
                  @validate="setValid('tableManagerCode', $event)"
                  :maxLength="100"
                  :required="true"
                  :errorLabel="$t('table_detail.code')"
                >
                </v-input>
              </div>
              <div class="form-group col l-7 md-7 c-7">
                <v-input
                  :label="$t('table_detail.name')"
                  v-model="tableManger.tableManagerName"
                  ref="tableManagerName"
                  @validate="setValid('tableManagerName', $event)"
                  :maxLength="100"
                  :required="true"
                  :errorLabel="$t('table_detail.name')"
                >
                </v-input>
              </div>
      ...
      <template #footer__right>
        ...
        <v-button
          @click="saveHandler(Enum.ACTION.SAVE_AND_ADD)"
          :tooltip="
            $t('action_form.save_and_add') + Enum.KEY_DEFINE.CTRL_SHIFT_S
          "
        >
...
export default {
  ...
  computed: {
    ...
    checkDisableIsEmployee: {
      get() {
        if (
          this.tableManger.isManager == false &&
          this.tableManger.isEmployee == true
        ) {
          return true;
        } else {
          return false;
    ...
    checkDisableIsManager: {
      get() {
        if (
          this.tableManger.isEmployee == false &&
          this.tableManger.isManager == true
        ) {
          return true;
        } else {
          return false;
  ...
  methods: {
    ...
    async updateInventoryItem() {
      let self = this;
      let tableManagerEdit = {
        tableManagerID: self.tableManger.tableManagerID,
        tableManagerCode: self.tableManger.tableManagerCode,
        tableManagerName: self.tableManger.tableManagerName,
        modifiedDate: self.tableManger.modifiedDate,
        capacity: self.tableManger.capacity,
      };
      const response = await self.$api.table.updateTable(tableManagerEdit); // gọi api update nhân viên
    ...
    async resetForm() {
      ...
      self.tableManger = {
        // gán giá trị mặc định cho tableManger
        tableManagerCode: "",
        tableManagerName: "",
        capacity: 0,
      };
