            ...
            <template #title>
                <div class="row e-header">
                    <div class="e-header__title col font-weight-700">
                        {{ isAdd ? $t('stock_infor.title_add') :  $t('stock_infor.title_out')}}
                    </div>
                </div>
            </template>
            <template #body>
                        ...
                        <div class="col l-6 md-6">
                            <div class="row sm-gutter">
                                <div class="form-group col l-5 md-5 c-5 focus">
                                    <v-input :label="$t('stock_infor.code')" v-model="inventoryItem.InventoryItemCode"
                                        ref="InventoryItemCode" @validate="setValid('InventoryItemCode', $event)" :maxLength="100"    
                                        :required="true" :errorLabel="$t('stock_infor.code')">
                                    </v-input>
                                </div>
                                <div class="form-group col l-7 md-7 c-7">
                                    <v-input :label="$t('stock_infor.name')" v-model="inventoryItem.InventoryItemName"    
                                        ref="InventoryItemName" @validate="setValid('InventoryItemName', $event)" :maxLength="100"
                                        :required="true" :errorLabel="$t('stock_infor.name')">
                                    </v-input>
                                </div>
                                <div class="form-group col l-12 md-12">
                                    <v-input 
                                        v-if="isAdd"
                                        :label="$t('stock_infor.user_add')" 
                                        v-model="inventoryItem.UserAdd">
                                    </v-input>
                                    <v-input 
                                        v-if="!isAdd"
                                        :label="$t('stock_infor.user_out')" 
                                        v-model="inventoryItem.UserOut"
                                        :maxLength="255">
                                    </v-input>
                                </div>
                            </div>
                        ...
                        <div class="col l-6 md-6">
                            <div class="row sm-gutter">
                                <div class="form-group col l-5 md-5">
                                    <v-date-picker 
                                        v-if="isAdd"
                                        :label="$t('stock_infor.date_add')"
                                        v-model="a" :isLessThanToday="true"
                                        :errorLabel="isAdd ? $t('stock_infor.date_add') : $t('stock_infor.date_out')" :validateCheck="true">
                                    </v-date-picker>
                                    <v-date-picker 
                                        v-if="!isAdd"
                                        :label="$t('stock_infor.date_out')"
                                        v-model="a" :isLessThanToday="true"
                                        :errorLabel="isAdd ? $t('stock_infor.date_add') : $t('stock_infor.date_out')" :validateCheck="true">
                                    </v-date-picker>
                                    <v-input 
                                        :label="$t('stock_infor.quantities')"
                                        v-model="inventoryItem.identityNumber" :isNumber="true"
                                    >
                                    </v-input>
                                </div>
                            </div>
    ...
    methods: {
        ...
        */
        ...
        async getEmployeeById(getNewEmployeeCode = false) {
            try {
                let self = this;
                const response = await self.$api.inventoryItem.getEmployeeById(self.getEmployeeId);
                if (response.status == Enum.MISA_CODE.SUCCESS) {
