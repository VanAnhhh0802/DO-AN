            ...
            <div class="col l-7 md-7">
              <div class="row sm-gutter">
                <div class="form-group col l-12 md-12">
                  <div class="v-input__label">
                    <label @click="handleInputFocus">
                      ...
                      {{ $t("inventory_info.choose_inventory") }}
                    </label>
                  </div>
                ...
                <div class="form-group col l-5 md-5 c-5 focus">
                  <v-input
                    :label="$t('inventory_info.code')"
                    v-model="inventory.inventoryCode"
                    disabled
                  >
                  </v-input>
                </div>
                ...
                <div class="form-group col l-12 md-12">
                  <v-input
                    :label="$t('inventory_info.inventory_category')"
                    v-model="inventory.inventoryCategoryName"
                    :maxLength="100"
                    disabled
                  >
                  </v-input>
                ...
                <div class="form-group col l-5 md-5">
                  <v-input
                    :label="$t('inventory_info.quantity_remaining')"
                    v-model="formatQuantity"
                    type="number"
                    disabled
                  >
                  </v-input>
                ...
                <div class="form-group col l-7 md-7">
                  <v-input
                    ...
                    @validate="handleValidatQuantityExprot"
                    :maxLength="10"
                  >
                  </v-input>
                </div>
...
export default {
  ...
  methods: {
    /**
     * Hàm xử lý khi chọn số lượng muốn xuất
     */
    handleValidatQuantityExprot() {
      const me = this;

      if (!me.quantityExport) return;

      if (me.quantityExport > me.inventory.quantity) {
    ...
    async resetForm(getNewInventoryCode = true) {
      try {
        ...
        if (response.status == Enum.MISA_CODE.SUCCESS) {
          me.inventory = {
            // gán giá trị mặc định cho inventory
    ...
    async getInventoryById(getNewInventoryCode = false) {
      try {
        let me = this;
        const response = await me.$api.inventory.getInventoryById(
          me.getInventoryId
        );
        if (response.status == Enum.MISA_CODE.SUCCESS) {
