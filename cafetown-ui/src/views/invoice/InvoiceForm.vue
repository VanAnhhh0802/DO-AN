  ...
  <div>
    ...
    <v-dialog :isShow="modelValue" @close="closeFormHandle">
      <template #title>
        <div class="row e-header">
          ...
          <div class="col">
            <v-input
              type="checkbox"
              :label_custom="$t('invoice_info.is_collected')"
              :disabled="this.isCollectedFirst"
              v-model="invoice.isCollected"
              :value="false"
            ></v-input>
          </div>
        </div>
            ...
            <div class="col l-6 md-6">
              <div class="row sm-gutter">
                <div class="form-group col l-12 md-12 c-12 focus">
                  <v-input
                    :label="$t('invoice_info.customer')"
                    v-model="invoice.customer"
                    :required="true"
                    :errorLabel="$t('invoice_info.customer')"
                  >
                  </v-input>
                </div>
            ...
            <div class="col l-6 md-6">
              <div class="row sm-gutter invoice--column">
                <div class="invoice__item--disable l-5 md-5">
                  <v-date-picker
                    :label="$t('invoice_info.purchase_date')"
                    v-model="invoice.purchaseDate"
                    :isLessThanToday="true"
                    :errorLabel="$t('invoice_info.purchase_date')"
                    :validateCheck="true"
                    :required="true"
                  >
                  </v-date-picker>
                ...
                <div class="invoice__item--disable l-5 md-5">
                  <v-input
                    :label="$t('invoice_info.invoice_code')"
                    :required="true"
                ...
                <div class="invoice__item--total">
                  <div class="invoice__total--label">
                    {{ $t("invoice_info.total_cost") }}
                  </div>
                  <div class="invoice__total--value">
                    {{
                      formatNumber(invoice.totalCost ? invoice.totalCost : 0)
                    }}
                  </div>
                </div>
              </div>
            ...
            <div class="table__header">
              <table>
                <thead>
                  ...
                  <th class="table__col table__col--left table__col--code">
                    {{ $t("invoice_info.code") }}
                  </th>
                  <th class="table__col table__col--left table__col--name">
                    {{ $t("invoice_info.name") }}
                  </th>
                  <th class="table__col table__col--left table__col--quantity">
                    {{ $t("invoice_info.quantity") }}
                  </th>
                  <th class="table__col table__col--left ttable__col--cost">
                    {{ $t("invoice_info.cost") }}
                  </th>
                  <th class="table__col table__col--left table__col--total">
                    {{ $t("invoice_info.total") }}
                  </th>
              ...
              <div class="grid__table--body">
                <table>
                  <tbody>
                    ...
                    >
                      <td
                        class="table__col table__col--center table__col--check"
                        :class="{ 'is--disabled': this.invoice.isCollected }"
                      >
                        {{ index + 1 }}
                      </td>
                      ...
                      <td
                        class="table__col table__col--left table__col--quantity table__col--spinner"
                      >
                        <input
                        ...
                        <span
                          class="spin spin--up"
                          @click="spinnerAction(invoice.inventoryID, true)"
                        ></span>
                        <span
                          class="spin spin--down"
                          @click="spinnerAction(invoice.inventoryID)"
                        ></span>
                      </td>
                      <td class="table__col table__col--left ttable__col--cost">
                        <p>
                          {{ formatCurrency(invoice.cost ? invoice.cost : 0) }}
                        </p>
                      </td>
                      <td class="table__col table__col--left table__col--total">
                        <p>{{ formatCurrency(invoice.totalCost) }}</p>
                      </td>
                      <td class="table__col--delete">
                        <div
                          class="icon--delete"
                          @click="
                            deleteInvoiceDetailByInventoryId(
                              invoice.inventoryID
                            )
                          "
                        ></div>
      ...
      <template #footer__right>
        <v-button
          @click="saveHandler(Enum.ACTION.SAVE_AND_CLOSE)"
          :disabled="this.isCollectedFirst"
          tooltipPosition="right"
  ...
  methods: {
    ...
    updateQuantity(inventoryID) {
      ...
      me.invoiceDetails.forEach((element, index) => {
        if (element.inventoryID == inventoryID) {
          ...
          if (
            me.invoiceDetails[index].quantity != 0 &&
            me.invoiceDetails[index].quantity
          ) {
      ...
      if (me.invoiceDetails && Array.isArray(me.invoiceDetails)) {
        me.invoiceDetails.forEach((element, index) => {
          if (element.inventoryID == inventoryID) {
            if (
              spinUp &&
              me.invoiceDetails[index].quantity != null &&
              me.invoiceDetails[index].quantity != undefined
            ) {
              me.inventories.forEach((inventory) => {
                if (inventory.inventoryID == inventoryID) {
    ...
    updateCost(index) {
      ...
      if (index != null && index != undefined) {
        me.invoiceDetails[index].totalCost =
          me.invoiceDetails[index].quantity * me.invoiceDetails[index].cost;
      }
      ...
      try {
        ...
        if (response.status == Enum.MISA_CODE.SUCCESS) {
          ...
          if (
            ...
            this.$store.getters.getPermission.employeeName
          ) {
            name = this.$store.getters.getPermission.employeeName;
          }

          me.invoice = {
            // gán giá trị mặc định cho invoice
    ...
    async getMasterDetailInvoiceByID() {
      try {
        let me = this;
        const response = await me.$api.invoice.getMasterDetailInvoiceByID(
          me.getInvoiceId
        );
        if (response.status == Enum.MISA_CODE.SUCCESS) {
    ...
    async saveHandler(action) {
      ...
      try {
        ...
        const validateResult = await me.$nextTick(async () => {
          // đợi cho khi các ô input validate xong thì mới tìm class error
          const errorClass = me.$el.querySelectorAll(".error");
          if (errorClass.length > 0) {
            // nếu có lỗi thì focus vào phần tử đầu tiên
            let count = 1;
            let htmlMessage = Array.from(errorClass)
              .map((item) => {
