            ...
            <div class="col l-6 md-6">
              <div class="row sm-gutter">
                <div class="form-group col l-5 md-5 c-5 focus">
                  <v-input
                    :label="$t('vendor_detail.code')"
                    v-model="vendor.vendorCode"
                    ref="vendorCode"
                    @validate="setValid('vendorCode', $event)"
                    :maxLength="100"
                    :required="true"
                    :errorLabel="$t('vendor_detail.code')"
                  >
                  </v-input>
                </div>
                <div class="form-group col l-7 md-7 c-7">
                  <v-input
                    :label="$t('vendor_detail.name')"
                    v-model="vendor.vendorName"
                    ref="vendorName"
                    @validate="setValid('vendorName', $event)"
                    :maxLength="100"
                    :required="true"
                    :errorLabel="$t('vendor_detail.name')"
                  >
                  </v-input>
                </div>
                <div class="form-group col l-12 md-12">
                  <v-input
                    :label="$t('vendor_detail.Address')"
                    v-model="vendor.address"
                    ref="address"
                    @validate="setValid('address', $event)"
                    :maxLength="100"
                    :required="true"
                    :errorLabel="$t('vendor_detail.Address')"
                  >
                  </v-input>
                  <v-input
                    :label="$t('vendor_detail.Phone')"
                    v-model="vendor.phone"
                    ref="phone"
                    @validate="setValid('phone', $event)"
                    :maxLength="100"
                    :required="true"
                    :errorLabel="$t('vendor_detail.Address')"
                  >
...
export default {
  ...
  computed: {
    ...
    checkDisableIsEmployee: {
      get() {
        if (
          this.inventoryItem.isManager == false &&
          this.inventoryItem.isEmployee == true
        ) {
          return true;
        } else {
          return false;
    ...
    checkDisableIsManager: {
      get() {
        if (
          this.inventoryItem.isEmployee == false &&
          this.inventoryItem.isManager == true
        ) {
          return true;
        } else {
          return false;
  ...
  methods: {
    ...
    async insertInventoryItem() {
      let self = this;
      if (self.vendor && self.vendor.vendorID) {
        delete self.vendor.vendorID;
      }
      ...
      if (response.status == Enum.MISA_CODE.CREATED) {
        ...
        self.$emit("insertVendor", self.vendor); // emit giá trị inventoryItem vừa thêm mới
        self.resetForm();
        return Promise.resolve(true);
      }
    },
